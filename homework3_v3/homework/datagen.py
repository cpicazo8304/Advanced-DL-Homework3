def generate_dataset(output_json: str, oversample: int = 10, temperature: float = 0.7):
    ...
    while len(results) < 1000:
      for i in range(0, len(data), 32):  # process 32 questions at a time
          if len(results) >= 1000:
              break
          batch = data[i:i+32]
          all_completions = llm.batched_generate(prompts[i:i+32], num_return_sequences=oversample, temperature=temperature)
